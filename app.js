        ...
        if(PFP !== 'avatar.jpg'){
            ...
            console.log(foundFile.Key + ' FOUND FILE')
              ...
              const params = {
                  ...
                  console.log(PFPurl + " THIS IS THE URL WOOHOO")
              }
            }
...
try{
    ...
    const Beat = await BT.find().sort({ createdAt: -1 })
        ...
        console.log(pname[0] + ' THIS IS PNAME')
         ...
         User = req.session.user
         res.render('Beats',{BT: Beats,BTs: Beat, PFPurl, productFiles,pname, FamPics,  urls,  UD: UDs,  Count: Counts, current: page,Favs, pages: Math.ceil(count / limit), Noti: Notis, FV: FVs,User, max, min, Susername, Orders })
}catch (err){
        ...
        .then((result) => {
            ...
            SC.find()
              .then((result) => {
                  ...
                  .then((result) => {
                    const UD = result
                    res.render('Withdraw',{ Noti, PFPurl, User, SB: ToTSB, Orders, UD })
                  })
